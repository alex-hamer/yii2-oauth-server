...
"repositories": [
    {
        "type": "vcs",
        "url": "git@gitlab.tariffstreet.com:mosaic/yii2-oauth2-server.git"
    }
],
```
...
`composer require mosaic/yii2-oauth2-server`
...
'modules' => [
    'oauth2' => [
        'class' => 'mosaic\yii2\oauth2server\Module',
...
We need to set up various tables to store our oauth details in, the migration can be found in `vendor/mosaic/yii2-oauth2-server/migrations`

Run `./yii migrate/create add_oauth_tables` to create an empty migration and copy the class functions from `/vendor/mosaic/yii2-oauth2-server/migrations/m140501_075311_add_oauth2_server.php` to the new file. 
...
use mosaic\yii2\oauth2server\filters\ErrorToExceptionFilter;
use mosaic\yii2\oauth2server\filters\auth\CompositeAuth;
